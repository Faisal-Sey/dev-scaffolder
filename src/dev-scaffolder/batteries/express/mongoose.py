...
from constants.backend.javascript.express.base import (
    EXPRESS_MONGOOSE_IMPORT,
    EXPRESS_MONGOOSE_SETUP,
)
from typings.base import ExecutorResponseStatus


class ExpressMongooseBattery(BaseBattery):
    """
    Battery that adds Mongoose (MongoDB ODM) to an Express app.
    ...
    def configure(self, project_path: str, project_name: str, app_name: str) -> None:
        ...
        try:
            with open(app_js, 'r', encoding='utf-8') as f:
                content = f.read()
            setup = EXPRESS_MONGOOSE_SETUP.replace('{project_name}', project_name)
            content = content.replace(
                '// [BATTERY:IMPORTS]',
                f'{EXPRESS_MONGOOSE_IMPORT}// [BATTERY:IMPORTS]',
            )
            content = content.replace(
                '// [BATTERY:MIDDLEWARE]',
                f'{setup}// [BATTERY:MIDDLEWARE]',
            )
