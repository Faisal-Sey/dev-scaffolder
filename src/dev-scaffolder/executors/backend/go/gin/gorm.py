...
class GinGormExecutor(BaseExecutor):
    """Scaffolds a Gin project with GORM and SQLite."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'go.mod'), GIN_GORM_GO_MOD)
        write(os.path.join(project_path, 'main.go'), GIN_GORM_MAIN_GO)
        write(os.path.join(project_path, '.gitignore'), GIN_GITIGNORE)
        write(os.path.join(project_path, '.env'), GIN_GORM_ENV)
        write(os.path.join(project_path, '.env.example'), GIN_GORM_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Gin GORM project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Gin project with GORM and SQLite scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_gin_gorm_template(**kwargs) -> ExecutorResponseStatus:
    return GinGormExecutor().run(**kwargs)


if __name__ == '__main__':
    args = GinGormExecutor.build_arg_parser().parse_args()
    GinGormExecutor().run(
