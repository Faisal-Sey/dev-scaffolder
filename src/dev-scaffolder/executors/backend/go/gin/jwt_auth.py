...
class GinJwtAuthExecutor(BaseExecutor):
    """Scaffolds a Gin project with JWT authentication."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'go.mod'), GIN_JWT_GO_MOD)
        write(os.path.join(project_path, 'main.go'), GIN_JWT_AUTH_MAIN_GO)
        write(os.path.join(project_path, '.gitignore'), GIN_GITIGNORE)
        write(os.path.join(project_path, '.env'), GIN_JWT_ENV)
        write(os.path.join(project_path, '.env.example'), GIN_JWT_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Gin JWT Auth project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Gin project with JWT authentication scaffolded with dev-scaffolder.\n\n'
            '## Auth Endpoints\n\n'
...
def generate_gin_jwt_auth_template(**kwargs) -> ExecutorResponseStatus:
    return GinJwtAuthExecutor().run(**kwargs)


if __name__ == '__main__':
    args = GinJwtAuthExecutor.build_arg_parser().parse_args()
    GinJwtAuthExecutor().run(
