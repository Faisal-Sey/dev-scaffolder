...
class GinOfficialExecutor(BaseExecutor):
    """Scaffolds a Gin project with basic health endpoint."""
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Gin project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Gin project scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_gin_official_template(**kwargs) -> ExecutorResponseStatus:
    return GinOfficialExecutor().run(**kwargs)


if __name__ == '__main__':
    args = GinOfficialExecutor.build_arg_parser().parse_args()
    GinOfficialExecutor().run(
