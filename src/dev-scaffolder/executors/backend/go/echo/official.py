...
class EchoOfficialExecutor(BaseExecutor):
    """Scaffolds an Echo v4 project with basic health endpoint."""
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Echo project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Echo v4 project scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_echo_official_template(**kwargs) -> ExecutorResponseStatus:
    return EchoOfficialExecutor().run(**kwargs)


if __name__ == '__main__':
    args = EchoOfficialExecutor.build_arg_parser().parse_args()
    EchoOfficialExecutor().run(
