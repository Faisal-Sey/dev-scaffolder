...
class EchoTestingExecutor(BaseExecutor):
    """Scaffolds an Echo v4 project with built-in integration tests."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'main.go'), ECHO_TESTING_MAIN_GO)
        write(os.path.join(project_path, 'main_test.go'), ECHO_TESTING_MAIN_TEST_GO)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Echo testing project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Echo v4 project with integration tests scaffolded with dev-scaffolder.\n\n'
            '## Run Tests\n\n'
            '```bash\n'
            'go test ./...\n'
            '```\n\n'
...
def generate_echo_testing_template(**kwargs) -> ExecutorResponseStatus:
    return EchoTestingExecutor().run(**kwargs)


if __name__ == '__main__':
    args = EchoTestingExecutor.build_arg_parser().parse_args()
    EchoTestingExecutor().run(
