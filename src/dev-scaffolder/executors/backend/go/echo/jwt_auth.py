...
class EchoJwtAuthExecutor(BaseExecutor):
    """Scaffolds an Echo v4 project with JWT authentication."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'go.mod'), ECHO_JWT_GO_MOD)
        write(os.path.join(project_path, 'main.go'), ECHO_JWT_AUTH_MAIN_GO)
        write(os.path.join(project_path, '.gitignore'), ECHO_GITIGNORE)
        write(os.path.join(project_path, '.env'), ECHO_JWT_ENV)
        write(os.path.join(project_path, '.env.example'), ECHO_JWT_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Echo JWT Auth project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Echo v4 project with JWT authentication scaffolded with dev-scaffolder.\n\n'
            '## Auth Endpoints\n\n'
...
def generate_echo_jwt_auth_template(**kwargs) -> ExecutorResponseStatus:
    return EchoJwtAuthExecutor().run(**kwargs)


if __name__ == '__main__':
    args = EchoJwtAuthExecutor.build_arg_parser().parse_args()
    EchoJwtAuthExecutor().run(
