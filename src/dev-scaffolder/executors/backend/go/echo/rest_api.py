...
class EchoRestAPIExecutor(BaseExecutor):
    """Scaffolds an Echo v4 REST API project with in-memory CRUD."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'main.go'), ECHO_REST_API_MAIN_GO)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Echo REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Echo v4 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_echo_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return EchoRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = EchoRestAPIExecutor.build_arg_parser().parse_args()
    EchoRestAPIExecutor().run(
