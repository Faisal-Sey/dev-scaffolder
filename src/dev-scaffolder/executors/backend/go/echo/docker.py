...
class EchoDockerExecutor(BaseExecutor):
    """Scaffolds an Echo v4 project with Docker support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'go.mod'), ECHO_GO_MOD)
        write(os.path.join(project_path, 'main.go'), ECHO_MAIN_GO)
        write(os.path.join(project_path, 'Dockerfile'), ECHO_DOCKERFILE)
        write(os.path.join(project_path, 'docker-compose.yml'), ECHO_DOCKER_COMPOSE_YML)
        write(os.path.join(project_path, '.dockerignore'), ECHO_DOCKERIGNORE)
        write(os.path.join(project_path, '.gitignore'), ECHO_GITIGNORE)
        write(os.path.join(project_path, '.env'), ECHO_ENV)
        write(os.path.join(project_path, '.env.example'), ECHO_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Echo Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Echo v4 project with Docker support scaffolded with dev-scaffolder.\n\n'
            '## Run with Docker\n\n'
            '```bash\n'
...
def generate_echo_docker_template(**kwargs) -> ExecutorResponseStatus:
    return EchoDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = EchoDockerExecutor.build_arg_parser().parse_args()
    EchoDockerExecutor().run(
