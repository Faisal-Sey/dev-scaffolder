...
class ActixOfficialExecutor(BaseExecutor):
    """Scaffolds a base Actix-web 4 project."""
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Actix-web project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Actix-web 4 project scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Rust 1.70+\n\n'
            '## Run\n\n'
            '```bash\n'
...
def generate_actix_official_template(**kwargs) -> ExecutorResponseStatus:
    return ActixOfficialExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ActixOfficialExecutor.build_arg_parser().parse_args()
    ActixOfficialExecutor().run(
