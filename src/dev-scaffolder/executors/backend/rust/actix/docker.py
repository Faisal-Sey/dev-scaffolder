...
class ActixDockerExecutor(BaseExecutor):
    """Scaffolds an Actix-web 4 project with Docker support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'Cargo.toml'), ACTIX_CARGO_TOML)
        write(os.path.join(src, 'main.rs'), ACTIX_MAIN_RS)
        write(os.path.join(project_path, 'Dockerfile'), ACTIX_DOCKERFILE)
        write(os.path.join(project_path, 'docker-compose.yml'), ACTIX_DOCKER_COMPOSE_YML)
        write(os.path.join(project_path, '.dockerignore'), ACTIX_DOCKERIGNORE)
        write(os.path.join(project_path, '.gitignore'), ACTIX_GITIGNORE)
        write(os.path.join(project_path, '.env'), ACTIX_ENV)
        write(os.path.join(project_path, '.env.example'), ACTIX_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Actix-web Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Actix-web 4 project with Docker support scaffolded with dev-scaffolder.\n\n'
            '## Run with Docker\n\n'
            '```bash\n'
...
def generate_actix_docker_template(**kwargs) -> ExecutorResponseStatus:
    return ActixDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ActixDockerExecutor.build_arg_parser().parse_args()
    ActixDockerExecutor().run(
