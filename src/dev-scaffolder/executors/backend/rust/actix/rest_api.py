...
class ActixRestAPIExecutor(BaseExecutor):
    """Scaffolds an Actix-web 4 REST API project with in-memory CRUD."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(src, 'main.rs'), ACTIX_REST_API_MAIN_RS)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Actix-web REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Actix-web 4 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_actix_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return ActixRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ActixRestAPIExecutor.build_arg_parser().parse_args()
    ActixRestAPIExecutor().run(
