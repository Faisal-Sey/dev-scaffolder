...
class ActixJwtAuthExecutor(BaseExecutor):
    """Scaffolds an Actix-web 4 project with JWT authentication."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'Cargo.toml'), ACTIX_JWT_AUTH_CARGO_TOML)
        write(os.path.join(src, 'main.rs'), ACTIX_JWT_AUTH_MAIN_RS)
        write(os.path.join(project_path, '.gitignore'), ACTIX_GITIGNORE)
        write(os.path.join(project_path, '.env'), ACTIX_JWT_ENV)
        write(os.path.join(project_path, '.env.example'), ACTIX_JWT_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Actix-web JWT Auth project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Actix-web 4 project with JWT authentication scaffolded with dev-scaffolder.\n\n'
            '## Auth Endpoints\n\n'
...
def generate_actix_jwt_auth_template(**kwargs) -> ExecutorResponseStatus:
    return ActixJwtAuthExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ActixJwtAuthExecutor.build_arg_parser().parse_args()
    ActixJwtAuthExecutor().run(
