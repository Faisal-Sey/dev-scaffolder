...
class ActixTestingExecutor(BaseExecutor):
    """Scaffolds an Actix-web 4 project with built-in integration tests."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(src, 'main.rs'), ACTIX_TESTING_MAIN_RS)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Actix-web testing project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Actix-web 4 project with integration tests scaffolded with dev-scaffolder.\n\n'
            '## Run Tests\n\n'
            '```bash\n'
            'cargo test\n'
            '```\n\n'
...
def generate_actix_testing_template(**kwargs) -> ExecutorResponseStatus:
    return ActixTestingExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ActixTestingExecutor.build_arg_parser().parse_args()
    ActixTestingExecutor().run(
