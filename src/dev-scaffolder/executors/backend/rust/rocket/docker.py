...
class RocketDockerExecutor(BaseExecutor):
    """Scaffolds a Rocket 0.5 project with Docker support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(src, 'main.rs'), ROCKET_MAIN_RS)
        write(os.path.join(project_path, 'Dockerfile'), ROCKET_DOCKERFILE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Rocket Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Rocket 0.5 project with Docker support scaffolded with dev-scaffolder.\n\n'
            '## Run with Docker\n\n'
            '```bash\n'
...
def generate_rocket_docker_template(**kwargs) -> ExecutorResponseStatus:
    return RocketDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = RocketDockerExecutor.build_arg_parser().parse_args()
    RocketDockerExecutor().run(
