...
class RocketOfficialExecutor(BaseExecutor):
    """Scaffolds a Rocket 0.5 project with a basic health endpoint."""
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Rocket project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Rocket 0.5 project scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_rocket_official_template(**kwargs) -> ExecutorResponseStatus:
    return RocketOfficialExecutor().run(**kwargs)


if __name__ == '__main__':
    args = RocketOfficialExecutor.build_arg_parser().parse_args()
    RocketOfficialExecutor().run(
