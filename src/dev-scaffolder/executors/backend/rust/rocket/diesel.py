...
class RocketDieselExecutor(BaseExecutor):
    """Scaffolds a Rocket 0.5 project with Diesel ORM and SQLite."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'Cargo.toml'), ROCKET_DIESEL_CARGO_TOML)
        write(os.path.join(src, 'main.rs'), ROCKET_DIESEL_MAIN_RS)
        write(os.path.join(src, 'schema.rs'), ROCKET_DIESEL_SCHEMA_RS)
        write(os.path.join(project_path, 'Dockerfile'), ROCKET_DIESEL_DOCKERFILE)
        write(os.path.join(project_path, 'docker-compose.yml'), ROCKET_DOCKER_COMPOSE_YML)
        write(os.path.join(project_path, '.dockerignore'), ROCKET_DOCKERIGNORE)
        write(os.path.join(project_path, 'Rocket.toml'), ROCKET_TOML)
        write(os.path.join(project_path, '.gitignore'), ROCKET_GITIGNORE)
        write(os.path.join(project_path, '.env'), ROCKET_DIESEL_ENV)
        write(os.path.join(project_path, '.env.example'), ROCKET_DIESEL_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Rocket Diesel project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Rocket 0.5 project with Diesel ORM and SQLite scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
            ...
            '```bash\n'
            'cp .env.example .env\n'
            '# Edit DATABASE_URL in .env (default: :memory:)\n'
            'cargo run\n'
            '```\n\n'
            '## Run with Docker\n\n'
            '```bash\n'
...
def generate_rocket_diesel_template(**kwargs) -> ExecutorResponseStatus:
    return RocketDieselExecutor().run(**kwargs)


if __name__ == '__main__':
    args = RocketDieselExecutor.build_arg_parser().parse_args()
    RocketDieselExecutor().run(
