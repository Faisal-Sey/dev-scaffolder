...
class RocketRestAPIExecutor(BaseExecutor):
    """Scaffolds a Rocket 0.5 REST API project with in-memory CRUD."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(src, 'main.rs'), ROCKET_REST_API_MAIN_RS)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Rocket REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Rocket 0.5 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_rocket_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return RocketRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = RocketRestAPIExecutor.build_arg_parser().parse_args()
    RocketRestAPIExecutor().run(
