...
class MicronautRestAPIExecutor(BaseExecutor):
    """Scaffolds a Micronaut 4 REST API project with CRUD endpoints."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        for d in [main_java, items_java, main_res, test_java]:
        ...
        write(os.path.join(items_java, 'Item.java'), MICRONAUT_ITEM_JAVA)
        write(os.path.join(items_java, 'ItemController.java'), MICRONAUT_ITEM_CONTROLLER_JAVA)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Micronaut REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Micronaut 4 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_micronaut_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return MicronautRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = MicronautRestAPIExecutor.build_arg_parser().parse_args()
    MicronautRestAPIExecutor().run(
