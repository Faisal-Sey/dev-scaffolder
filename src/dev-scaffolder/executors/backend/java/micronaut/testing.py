...
class MicronautTestingExecutor(BaseExecutor):
    """Scaffolds a Micronaut 4 project with HTTP client integration tests."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(test_java, 'HealthControllerTest.java'), MICRONAUT_HEALTH_CONTROLLER_TEST_JAVA)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Micronaut testing project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Micronaut 4 project with HTTP client integration tests scaffolded by dev-scaffolder.\n\n'
            '## Run Tests\n\n'
            '```bash\n'
            'mvn test\n'
            '```\n\n'
...
def generate_micronaut_testing_template(**kwargs) -> ExecutorResponseStatus:
    return MicronautTestingExecutor().run(**kwargs)


if __name__ == '__main__':
    args = MicronautTestingExecutor.build_arg_parser().parse_args()
    MicronautTestingExecutor().run(
