...
class SpringBootWebSocketsExecutor(BaseExecutor):
    """Scaffolds a Spring Boot 3 project with WebSocket (STOMP) support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(ws_java, 'WebSocketConfig.java'), SPRING_BOOT_WEBSOCKET_CONFIG_JAVA)
        write(os.path.join(ws_java, 'ChatMessage.java'), SPRING_BOOT_CHAT_MESSAGE_JAVA)
        write(os.path.join(ws_java, 'MessageController.java'), SPRING_BOOT_MESSAGE_CONTROLLER_JAVA)
        ...
        write(os.path.join(project_path, 'pom.xml'), SPRING_BOOT_WEBSOCKET_POM_XML)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Spring Boot WebSocket project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Spring Boot 3 project with WebSocket (STOMP) support scaffolded by dev-scaffolder.\n\n'
            '## WebSocket Endpoint\n\n'
            'Connect via SockJS at `/ws`, then subscribe to `/topic/messages` and send to `/app/chat`.\n\n'
            '## Run\n\n'
            '```bash\n'
            'mvn spring-boot:run\n'
            '```\n\n'
...
def generate_spring_boot_websockets_template(**kwargs) -> ExecutorResponseStatus:
    return SpringBootWebSocketsExecutor().run(**kwargs)


if __name__ == '__main__':
    args = SpringBootWebSocketsExecutor.build_arg_parser().parse_args()
    SpringBootWebSocketsExecutor().run(
