...
class SpringBootGraphQLExecutor(BaseExecutor):
    """Scaffolds a Spring Boot 3 project with GraphQL support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        graphql_res = os.path.join(main_res, 'graphql')
        test_java = os.path.join(project_path, 'src', 'test', 'java', 'com', 'example', pkg)

        for d in [main_java, items_java, main_res, graphql_res, test_java]:
        ...
        write(os.path.join(items_java, 'Item.java'), SPRING_BOOT_GRAPHQL_ITEM_JAVA)
        write(os.path.join(items_java, 'ItemController.java'), SPRING_BOOT_GRAPHQL_CONTROLLER_JAVA)
        write(os.path.join(graphql_res, 'schema.graphqls'), SPRING_BOOT_GRAPHQL_SCHEMA)
        write(os.path.join(main_res, 'application.properties'), SPRING_BOOT_GRAPHQL_APPLICATION_PROPERTIES)
        write(os.path.join(test_java, 'ApplicationTests.java'), SPRING_BOOT_APPLICATION_TEST_JAVA)
        write(os.path.join(project_path, 'pom.xml'), SPRING_BOOT_GRAPHQL_POM_XML)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Spring Boot GraphQL project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Spring Boot 3 project with GraphQL support scaffolded by dev-scaffolder.\n\n'
            '## GraphQL Endpoint\n\n'
            '`POST /graphql`\n\n'
            '## GraphiQL IDE\n\n'
            ...
            '```graphql\n'
            '# List items\n'
            'query { items { id title description } }\n\n'
            '# Create item\n'
            'mutation { createItem(title: "Hello", description: "World") { id title } }\n\n'
            '# Delete item\n'
...
def generate_spring_boot_graphql_template(**kwargs) -> ExecutorResponseStatus:
    return SpringBootGraphQLExecutor().run(**kwargs)


if __name__ == '__main__':
    args = SpringBootGraphQLExecutor.build_arg_parser().parse_args()
    SpringBootGraphQLExecutor().run(
