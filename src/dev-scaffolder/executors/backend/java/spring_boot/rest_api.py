...
class SpringBootRestAPIExecutor(BaseExecutor):
    """Scaffolds a Spring Boot 3 REST API project with CRUD endpoints."""
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Spring Boot REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Spring Boot 3 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_spring_boot_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return SpringBootRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = SpringBootRestAPIExecutor.build_arg_parser().parse_args()
    SpringBootRestAPIExecutor().run(
