...
class QuarkusTestingExecutor(BaseExecutor):
    """Scaffolds a Quarkus 3 project with REST-assured integration tests."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(test_java, 'GreetingResourceTest.java'), QUARKUS_GREETING_RESOURCE_TEST_JAVA)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Quarkus testing project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Quarkus 3 project with REST-assured integration tests scaffolded by dev-scaffolder.\n\n'
            '## Run Tests\n\n'
            '```bash\n'
            'mvn test\n'
            '```\n\n'
...
def generate_quarkus_testing_template(**kwargs) -> ExecutorResponseStatus:
    return QuarkusTestingExecutor().run(**kwargs)


if __name__ == '__main__':
    args = QuarkusTestingExecutor.build_arg_parser().parse_args()
    QuarkusTestingExecutor().run(
