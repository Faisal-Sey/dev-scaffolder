...
class QuarkusDockerExecutor(BaseExecutor):
    """Scaffolds a Quarkus 3 project with Docker support."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(project_path, 'Dockerfile'), QUARKUS_DOCKERFILE_JVM)
        write(os.path.join(project_path, 'docker-compose.yml'), QUARKUS_DOCKER_COMPOSE_YML)
        write(os.path.join(project_path, '.dockerignore'), QUARKUS_DOCKERIGNORE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Quarkus Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Quarkus 3 project with Docker support scaffolded by dev-scaffolder.\n\n'
            '## Run with Docker\n\n'
            '```bash\n'
...
def generate_quarkus_docker_template(**kwargs) -> ExecutorResponseStatus:
    return QuarkusDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = QuarkusDockerExecutor.build_arg_parser().parse_args()
    QuarkusDockerExecutor().run(
