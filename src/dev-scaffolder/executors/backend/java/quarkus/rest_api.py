...
class QuarkusRestAPIExecutor(BaseExecutor):
    """Scaffolds a Quarkus 3 REST API project with CRUD endpoints."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(items_java, 'Item.java'), QUARKUS_ITEM_JAVA)
        write(os.path.join(items_java, 'ItemResource.java'), QUARKUS_ITEM_RESOURCE_JAVA)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Quarkus REST API '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Quarkus 3 REST API scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
...
def generate_quarkus_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return QuarkusRestAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = QuarkusRestAPIExecutor.build_arg_parser().parse_args()
    QuarkusRestAPIExecutor().run(
