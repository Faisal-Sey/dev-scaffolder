...
class QuarkusJwtAuthExecutor(BaseExecutor):
    """Scaffolds a Quarkus 3 project with SmallRye JWT authentication."""
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        write(os.path.join(auth_java, 'AuthRequest.java'), QUARKUS_AUTH_REQUEST_JAVA)
        write(os.path.join(auth_java, 'AuthResponse.java'), QUARKUS_AUTH_RESPONSE_JAVA)
        write(os.path.join(auth_java, 'AuthResource.java'), QUARKUS_AUTH_RESOURCE_JAVA)
        write(os.path.join(main_res, 'application.properties'), QUARKUS_JWT_APPLICATION_PROPERTIES)
        write(os.path.join(project_path, 'pom.xml'), QUARKUS_JWT_POM_XML)
        write(os.path.join(project_path, '.gitignore'), QUARKUS_GITIGNORE)
        write(os.path.join(project_path, '.env'), QUARKUS_JWT_ENV)
        write(os.path.join(project_path, '.env.example'), QUARKUS_JWT_ENV_EXAMPLE)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Quarkus JWT Auth project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Quarkus 3 project with SmallRye JWT authentication scaffolded by dev-scaffolder.\n\n'
            '## Auth Endpoints\n\n'
...
def generate_quarkus_jwt_auth_template(**kwargs) -> ExecutorResponseStatus:
    return QuarkusJwtAuthExecutor().run(**kwargs)


if __name__ == '__main__':
    args = QuarkusJwtAuthExecutor.build_arg_parser().parse_args()
    QuarkusJwtAuthExecutor().run(
