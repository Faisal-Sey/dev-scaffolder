...
class NestJSTestingExecutor(BaseExecutor):
    """
    Executor that scaffolds a NestJS project with comprehensive test setup.

    Includes unit tests (app.controller.spec.ts) and e2e tests using Jest
    and Supertest. The e2e suite bootstraps the full NestJS app and validates
    the root and /health endpoints end-to-end.
    """
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing NestJS + testing dependencies...[/bold blue]')
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        files = {
            ...
            'src/app.module.ts': NESTJS_APP_MODULE_TS,
            'src/app.controller.ts': NESTJS_APP_CONTROLLER_TS,
            'src/app.controller.spec.ts': NESTJS_APP_CONTROLLER_SPEC_TS,
            'test/app.e2e-spec.ts': NESTJS_E2E_TEST_TS,
            'test/jest-e2e.json': NESTJS_JEST_E2E_JSON,
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]NestJS + testing project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A NestJS project with full test setup scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
            '## Setup\n\n'
...
def generate_nestjs_testing_template(**kwargs) -> ExecutorResponseStatus:
    return NestJSTestingExecutor().run(**kwargs)


if __name__ == '__main__':
    args = NestJSTestingExecutor.build_arg_parser().parse_args()
    NestJSTestingExecutor().run(
