...
class NestJSWebSocketsExecutor(BaseExecutor):
    """
    Executor that scaffolds a NestJS project with Socket.io WebSockets.

    Adds an EventsModule with an EventsGateway that handles:
      - 'message' events: broadcasts to all connected clients
      - connection/disconnection logging and user count updates ('users' event)
    """
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing NestJS + WebSocket dependencies...[/bold blue]')
        npm = self.get_venv_environment()
        deps = [
            '@nestjs/common', '@nestjs/core', '@nestjs/platform-express',
            '@nestjs/platform-socket.io', '@nestjs/websockets',
            'dotenv', 'reflect-metadata', 'rxjs', 'socket.io',
        ]
        dev_deps = [
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        os.makedirs(os.path.join(project_path, 'src', 'events'), exist_ok=True)
        os.makedirs(os.path.join(project_path, 'test'), exist_ok=True)

        files = {
            'src/main.ts': NESTJS_MAIN_TS,
            'src/app.module.ts': NESTJS_WEBSOCKETS_APP_MODULE_TS,
            'src/app.controller.ts': NESTJS_APP_CONTROLLER_TS,
            'src/app.controller.spec.ts': NESTJS_APP_CONTROLLER_SPEC_TS,
            'src/events/events.gateway.ts': NESTJS_EVENTS_GATEWAY_TS,
            'src/events/events.module.ts': NESTJS_EVENTS_MODULE_TS,
    ...
    def _write_package_json(self, project_path: str, project_name: str) -> None:
        pkg = NESTJS_WEBSOCKETS_PACKAGE_JSON.replace('{project_name}', project_name)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]NestJS + WebSockets project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A NestJS project with Socket.io WebSockets scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
            '## Setup\n\n'
            ...
            '|-------|-----------|-------------|\n'
            '| `message` | client → server | Broadcast message to all clients |\n'
            '| `message` | server → client | Receive broadcast message |\n'
            '| `users` | server → client | Connected user count update |\n\n'
...
def generate_nestjs_websockets_template(**kwargs) -> ExecutorResponseStatus:
    return NestJSWebSocketsExecutor().run(**kwargs)


if __name__ == '__main__':
    args = NestJSWebSocketsExecutor.build_arg_parser().parse_args()
    NestJSWebSocketsExecutor().run(
