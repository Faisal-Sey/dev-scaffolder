...
class NestJSDockerExecutor(BaseExecutor):
    """
    Executor that scaffolds a NestJS project with Docker support.

    Adds a multi-stage Dockerfile (build → production), docker-compose.yml
    for local development with live-reload, and a .dockerignore.
    """
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        files = {
            ...
            'Dockerfile': NESTJS_DOCKERFILE,
            'docker-compose.yml': NESTJS_DOCKER_COMPOSE,
            '.dockerignore': NESTJS_DOCKERIGNORE,
        }
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]NestJS + Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A NestJS project with Docker support scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
...
def generate_nestjs_docker_template(**kwargs) -> ExecutorResponseStatus:
    return NestJSDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = NestJSDockerExecutor.build_arg_parser().parse_args()
    NestJSDockerExecutor().run(
