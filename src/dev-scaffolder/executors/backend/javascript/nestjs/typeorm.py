...
class NestJSTypeORMExecutor(BaseExecutor):
    """
    Executor that scaffolds a NestJS project with TypeORM (better-sqlite3).

    Adds TypeOrmModule.forRoot() with a SQLite database, a User entity, and
    a UsersModule with full CRUD over /users. Switch to postgres/mysql by
    updating the forRoot() config and installing the matching driver.
    """
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing NestJS + TypeORM dependencies...[/bold blue]')
        npm = self.get_venv_environment()
        deps = [
            '@nestjs/common', '@nestjs/core', '@nestjs/platform-express',
            '@nestjs/typeorm', 'better-sqlite3',
            'dotenv', 'reflect-metadata', 'rxjs', 'typeorm',
        ]
        dev_deps = [
            '@nestjs/cli', '@nestjs/schematics', '@nestjs/testing',
            '@types/better-sqlite3', '@types/jest', '@types/node',
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        for d in ['src/users', 'data', 'test']:
            os.makedirs(os.path.join(project_path, d), exist_ok=True)

        files = {
            'src/main.ts': NESTJS_MAIN_TS,
            'src/app.module.ts': NESTJS_TYPEORM_APP_MODULE_TS,
            'src/app.controller.ts': NESTJS_APP_CONTROLLER_TS,
            'src/app.controller.spec.ts': NESTJS_APP_CONTROLLER_SPEC_TS,
            'src/users/user.entity.ts': NESTJS_USER_ENTITY_TS,
            'src/users/users.module.ts': NESTJS_TYPEORM_USERS_MODULE_TS,
            'src/users/users.service.ts': NESTJS_TYPEORM_USERS_SERVICE_TS,
            'src/users/users.controller.ts': NESTJS_TYPEORM_USERS_CONTROLLER_TS,
    ...
    def _write_package_json(self, project_path: str, project_name: str) -> None:
        pkg = NESTJS_TYPEORM_PACKAGE_JSON.replace('{project_name}', project_name)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]NestJS + TypeORM project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A NestJS project with TypeORM (SQLite) scaffolded with dev-scaffolder.\n\n'
            '## Endpoints\n\n'
            ...
            'npm run start:dev\n'
            '```\n\n'
            'Database stored in `data/database.sqlite` (auto-created).\n'
...
def generate_nestjs_typeorm_template(**kwargs) -> ExecutorResponseStatus:
    return NestJSTypeORMExecutor().run(**kwargs)


if __name__ == '__main__':
    args = NestJSTypeORMExecutor.build_arg_parser().parse_args()
    NestJSTypeORMExecutor().run(
