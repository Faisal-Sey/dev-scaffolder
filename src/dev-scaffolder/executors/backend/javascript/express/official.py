...
class ExpressOfficialExecutor(BaseExecutor):
    """
    Executor that scaffolds a base Express.js project.
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing Express dependencies...[/bold blue]')
        for cmd in [
    ...
    def _write_package_json(self, project_path: str, project_name: str) -> None:
        pkg_content = EXPRESS_PACKAGE_JSON.replace('{project_name}', project_name)
    ...
    def _convert_package_manager(self, pm: str, project_path: str) -> None:
        """If the user chose yarn or pnpm, delete package-lock.json and reinstall."""
        if not pm or pm == 'npm':
            return
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Express project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Express.js project scaffolded with dev-scaffolder.\n'
            f'{batteries_note}\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
...
def generate_express_official_template(**kwargs) -> ExecutorResponseStatus:
    return ExpressOfficialExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ExpressOfficialExecutor.build_arg_parser().parse_args()
    ExpressOfficialExecutor().run(
