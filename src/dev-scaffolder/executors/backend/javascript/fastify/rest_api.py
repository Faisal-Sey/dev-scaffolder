...
class FastifyRESTAPIExecutor(BaseExecutor):
    """
    Executor that scaffolds a Fastify REST API project with a full CRUD resource.
      ...
      {project_name}/
        src/
          ...
          routes/
            ...
            items.js      -- GET/POST/PUT/DELETE /items (in-memory store)
        package.json
        .env
        .env.example
        .gitignore
    ...
    Replace the in-memory items store with a database of your choice.
    """
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        with open(os.path.join(project_path, 'src', 'routes', 'items.js'), 'w', encoding='utf-8') as f:
            f.write(FASTIFY_REST_ITEMS_ROUTES_JS)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Fastify REST API project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Fastify REST API scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
            '## Setup\n\n'
            ...
            'The items store is in-memory by default. '
            'Replace it with a database (Prisma, Mongoose, etc.) using batteries.\n'
        )
...
def generate_fastify_rest_api_template(**kwargs) -> ExecutorResponseStatus:
    return FastifyRESTAPIExecutor().run(**kwargs)


if __name__ == '__main__':
    args = FastifyRESTAPIExecutor.build_arg_parser().parse_args()
    FastifyRESTAPIExecutor().run(
