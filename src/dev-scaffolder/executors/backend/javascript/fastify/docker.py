...
class FastifyDockerExecutor(BaseExecutor):
    """
    Executor that scaffolds a Fastify project with Docker support.
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Fastify + Docker project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Fastify project with Docker support scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n'
...
def generate_fastify_docker_template(**kwargs) -> ExecutorResponseStatus:
    return FastifyDockerExecutor().run(**kwargs)


if __name__ == '__main__':
    args = FastifyDockerExecutor.build_arg_parser().parse_args()
    FastifyDockerExecutor().run(
