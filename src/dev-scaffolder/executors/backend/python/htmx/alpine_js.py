...
class HtmxAlpineJsExecutor(BaseExecutor):
    """
    Executor for scaffolding a Django + HTMX + Alpine.js project.

    Delegates base project setup to HtmxOfficialExecutor, then overwrites
    the templates with Alpine.js-enhanced versions. Supports optional batteries.
    """
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        """
        Scaffolds a Django + HTMX + Alpine.js project.
        ...
        self._update_status('[bold blue]Writing Alpine.js base.html...[/bold blue]')
        with open(os.path.join(templates_dir, 'base.html'), 'w', encoding='utf-8') as f:
            f.write(HTMX_ALPINE_BASE_HTML.format(project_name=project_name))

        self._update_status('[bold blue]Writing Alpine.js index.html...[/bold blue]')
        with open(os.path.join(templates_dir, 'index.html'), 'w', encoding='utf-8') as f:
            f.write(HTMX_ALPINE_INDEX_HTML.format(project_name=project_name))
        ...
        self.console.print(
            f"[bold green]Django + HTMX + Alpine.js project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A Django + HTMX + Alpine.js project scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Python 3.8+\n\n'
            '## Setup\n\n'
            '```bash\n'
            ...
            '- **Alpine.js** — lightweight client-side reactivity (CDN)\n'
        )

    def generate(self, **kwargs: DjangoOfficialTemplateArgs) -> DjangoOfficialTemplateResponse:
        """
        Generates the Django + HTMX + Alpine.js project template.
...
def generate_htmx_alpine_js_template(**kwargs) -> DjangoOfficialTemplateResponse:
    return HtmxAlpineJsExecutor().run(**kwargs)


if __name__ == '__main__':
    args = HtmxAlpineJsExecutor.build_arg_parser().parse_args()
    HtmxAlpineJsExecutor().run(
