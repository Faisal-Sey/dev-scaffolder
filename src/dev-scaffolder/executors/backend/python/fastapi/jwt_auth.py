...
class FastAPIJWTAuthExecutor(BaseExecutor):
    """
    Executor that scaffolds a FastAPI project with JWT authentication.

    Uses async SQLAlchemy (aiosqlite by default), passlib for password hashing,
    ...
    def install_dependencies(self, venv_python_executor: str) -> ExecutorResponseStatus:
        ...
        self.console.print('[bold green]FastAPI JWT Auth dependencies installed[/bold green]')
        return ExecutorResponseStatus(success=True)

    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        auth_dir = os.path.join(project_path, 'app', 'auth')
        os.makedirs(auth_dir, exist_ok=True)

        open(os.path.join(project_path, 'app', '__init__.py'), 'w').close()
        open(os.path.join(auth_dir, '__init__.py'), 'w').close()

        with open(os.path.join(project_path, 'app', 'main.py'), 'w') as f:
            f.write(FASTAPI_JWT_MAIN_PY.format(project_name=project_name))

        with open(os.path.join(project_path, 'app', 'database.py'), 'w') as f:
            f.write(FASTAPI_JWT_DATABASE_PY)

        with open(os.path.join(auth_dir, 'models.py'), 'w') as f:
            f.write(FASTAPI_JWT_USER_MODEL_PY)

        with open(os.path.join(auth_dir, 'schemas.py'), 'w') as f:
            f.write(FASTAPI_JWT_SCHEMAS_PY)

        with open(os.path.join(auth_dir, 'utils.py'), 'w') as f:
            f.write(FASTAPI_JWT_UTILS_PY)

        with open(os.path.join(auth_dir, 'router.py'), 'w') as f:
            f.write(FASTAPI_JWT_ROUTER_PY)

        with open(os.path.join(project_path, 'requirements.txt'), 'w') as f:
            f.write(FASTAPI_JWT_REQUIREMENTS)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self._update_status('[bold blue]Installing JWT Auth dependencies...[/bold blue]')
        ...
        self.console.print(
            f'[bold green]FastAPI JWT Auth project \'{project_name}\' created successfully![/bold green]'
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'A FastAPI project with JWT authentication, scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Python 3.10+\n\n'
            '## Setup\n\n'
            '```bash\n'
            ...
            '| GET  | `/auth/me` | Get current user (pass token as query param) |\n\n'
            '## Token Usage\n\n'
...
def generate_fastapi_jwt_auth_template(**kwargs) -> ExecutorResponseStatus:
    return FastAPIJWTAuthExecutor().run(**kwargs)


if __name__ == '__main__':
    args = FastAPIJWTAuthExecutor.build_arg_parser().parse_args()
    FastAPIJWTAuthExecutor().run(
