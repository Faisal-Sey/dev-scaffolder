...
class ExpressTSSocketIOExecutor(BaseExecutor):
    """
    Executor that scaffolds an Express.js + TypeScript project with Socket.IO.

    Project layout:
      {project_name}/
        src/
          index.ts    -- Creates http.Server and attaches Socket.IO
          app.ts      -- Express app (HTTP routes)
          socket.ts   -- Socket.IO setup with rooms and typed events
          routes/
            index.ts
        tsconfig.json
        package.json
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing Express + TypeScript + Socket.IO dependencies...[/bold blue]')
        for cmd in [
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        with open(os.path.join(project_path, 'src', 'index.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_SOCKETIO_INDEX_TS)

        with open(os.path.join(project_path, 'src', 'app.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_APP_TS)

        with open(os.path.join(project_path, 'src', 'socket.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_SOCKETIO_SOCKET_TS)
    ...
    def _write_package_json(self, project_path: str, project_name: str) -> None:
        pkg_content = EXPRESS_TS_SOCKETIO_PACKAGE_JSON.replace('{project_name}', project_name)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Express + TypeScript + Socket.IO project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Express.js + TypeScript + Socket.IO project scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
            '## Setup\n\n'
            ...
            'npm run dev\n'
            '```\n\n'
            'Server + Socket.IO at http://localhost:3000\n'
        )
...
def generate_express_ts_socket_io_template(**kwargs) -> ExecutorResponseStatus:
    return ExpressTSSocketIOExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ExpressTSSocketIOExecutor.build_arg_parser().parse_args()
    ExpressTSSocketIOExecutor().run(
