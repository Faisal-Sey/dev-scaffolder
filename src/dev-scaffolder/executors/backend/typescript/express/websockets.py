...
class ExpressTSWebSocketsExecutor(BaseExecutor):
    """
    Executor that scaffolds an Express.js + TypeScript project with WebSocket support.

    Project layout:
      {project_name}/
        src/
          index.ts       -- Creates http.Server and attaches WebSocket server
          app.ts         -- Express app (HTTP routes)
          websocket.ts   -- WebSocketServer with typed client management
          routes/
            index.ts
        tsconfig.json
        package.json
    ...
    def install_dependencies(self, project_path: str) -> ExecutorResponseStatus:
        self._update_status('[bold blue]Installing Express + TypeScript + ws dependencies...[/bold blue]')
        for cmd in [
            [shutil.which('npm') or 'npm', 'install', 'express', 'dotenv', 'ws'],
            [shutil.which('npm') or 'npm', 'install', '--save-dev',
             '@types/express', '@types/node', '@types/ws', 'typescript', 'tsx'],
        ]:
    ...
    def _create_project_structure(self, project_path: str, project_name: str) -> None:
        ...
        with open(os.path.join(project_path, 'src', 'index.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_WS_INDEX_TS)

        with open(os.path.join(project_path, 'src', 'app.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_APP_TS)

        with open(os.path.join(project_path, 'src', 'websocket.ts'), 'w', encoding='utf-8') as f:
            f.write(EXPRESS_TS_WS_WEBSOCKET_TS)
    ...
    def _write_package_json(self, project_path: str, project_name: str) -> None:
        pkg_content = EXPRESS_TS_WS_PACKAGE_JSON.replace('{project_name}', project_name)
    ...
    def execute_creation_commands(self, **kwargs) -> ExecutorResponseStatus:
        ...
        self.console.print(
            f"[bold green]Express + TypeScript + WebSockets project '{project_name}' created successfully![/bold green]"
        )
        return ExecutorResponseStatus(success=True)
    ...
    def get_readme_content(self, **kwargs) -> str:
        ...
        return (
            f'# {project_name}\n\n'
            'An Express.js + TypeScript + WebSockets project scaffolded with dev-scaffolder.\n\n'
            '## Requirements\n\n'
            '- Node.js 18+\n'
            '- npm\n\n'
            '## Setup\n\n'
...
def generate_express_ts_websockets_template(**kwargs) -> ExecutorResponseStatus:
    return ExpressTSWebSocketsExecutor().run(**kwargs)


if __name__ == '__main__':
    args = ExpressTSWebSocketsExecutor.build_arg_parser().parse_args()
    ExpressTSWebSocketsExecutor().run(
